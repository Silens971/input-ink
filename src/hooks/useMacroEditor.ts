 ...
 export function useMacroEditor(recordingData: RecordingData | null) {
   ...
   const generateAHK = useCallback((points: MousePoint[]): string => {
     ...
     const lines = [
       ...
       '; ===================================',
       '; ',
       '; Tempo ajustado: ' + (timeSettings.timeMultiplier !== 1 ? `${timeSettings.timeMultiplier.toFixed(2)}x` : 'Original'),
       '; Pontos: ' + points.length,
       '; Duração: ' + (points.length > 0 ? (points[points.length - 1].timestamp / 1000).toFixed(2) + 's' : '0s'),
       '; ',
       '',
   ...
   const generateLua = useCallback((points: MousePoint[]): string => {
     ...
     const lines = [
       ...
       '  -- Use a função de sleep do seu ambiente',
       'end',
       '',
       'local movements = {',
     ];
